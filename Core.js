...
module.exports = async (Miku, m, commands, chatUpdate, store) => {
  try {
    ...
    global.suppL = "https://cutt.ly/J4l7zmI";
    ...
    if (body.startsWith(prefix) && !icmd) {
      let peacetext = `No such command programmed *${pushname}* ! Type *${prefix}menu* or press the button below to get help!\n\n*${pushname}*, එහෙම command එකක් නැ. *${prefix}මෙනු* කියලා type කරන්න නැත්තම් පහල button එකක් click කරන්න.`;
    ...
    if (timestamps.has(m.sender)) {
      ...
      if (now < expiration) {
        ...
        return await Miku.sendMessage(
          m.from,
          {
            text: `Command Rejected ! Don't Spam ! You can use command after _${timeLeft.toFixed(1)} second(s)_\nවිධානය ප්‍රතික්ෂේප විය! ස්පෑම් නොකරන්න! ඔබට _${timeLeft.toFixed(1)} තත්පර(s)_ පසු විධානය භාවිතා කළ හැක`,
          },
          {
            quoted: m,
          }
