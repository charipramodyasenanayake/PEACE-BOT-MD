    ...
    if (args[0] === "on") {
      if (!checkdata) {
        ...
        Miku.sendMessage(
          m.from,
          {
            text: `ᴘᴇᴀᴄᴇ ᴍᴅ has been Re-Activated in this group!`,
        ...
        if (checkdata.botSwitch == "true")
          return Miku.sendMessage(
            m.from,
            { text: `ᴘᴇᴀᴄᴇ ᴍᴅ is already Activated in this group !` },
            { quoted: m }
          );
        await mk.updateOne({ id: m.from }, { botSwitch: "true" });
        return Miku.sendMessage(
          m.from,
          { text: `ᴘᴇᴀᴄᴇ ᴍᴅ has been Activated in this group! Now everyone here can use bot.` },
          { quoted: m }
        );
      }
    } else if (args[0] === "off") {
      if (!checkdata) {
        ...
        return Miku.sendMessage(
          m.from,
          {
            text: `ᴘᴇᴀᴄᴇ ᴍᴅ has been De-Activated in this group !\n\nNow only *Admins* can use bot`,
          },
          { quoted: m }
        );
      } else {
        if (checkdata.botSwitch == "false")
          return Miku.sendMessage(
            m.from,
            { text: `ᴘᴇᴀᴄᴇ ᴍᴅ is already De-Activated in this group !\n\nNow only *Admins* can use bot` },
            { quoted: m }
          );
